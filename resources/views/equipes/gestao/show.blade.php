...
@section('content')
    ...
    <div class="container">
        ...
        <form>
            <div class="row g-3">
                ...
                <div class="col-md-8">
                    <label for="descricao_show" class="form-label">Descrição</label>
                    <input type="text" class="form-control" name="descricao_show" id="descricao_show"
                        value="{{ $equipe->descricao }}" readonly>
                </div>
                <div class="col-md-4">
                    <label for="numero_show" class="form-label">Número</label>
                    <input type="text" class="form-control" name="numero_show" id="numero_show"
                        value="{{ $equipe->numero }}" readonly>
                </div>

                <div class="col-md-3">
                    <label for="cnes_show" class="form-label">CNES</label>
                    <input type="text" class="form-control" name="cnes_show" id="cnes_show" value="{{ $equipe->cnes }}"
                        readonly>
                </div>
                <div class="col-md-3">
                    <label for="ine_show" class="form-label">INE</label>
                    <input type="text" class="form-control" name="ine_show" id="ine_show" value="{{ $equipe->ine }}"
                        readonly>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control" name="tipo_show" id="tipo_show"
                        value="{{ $equipe->equipeTipo->nome }}" readonly>
                </div>

                <div class="col-md-8">
                    <label for="unidade_show" class="form-label">Unidade</label>
                    <input type="text" class="form-control" name="unidade_show" id="unidade_show"
                        value="{{ $equipe->unidade->nome }}" readonly>
                </div>
                <div class="col-md-4">
                    <label for="distrito_show" class="form-label">Distrito</label>
                    <input type="text" class="form-control" name="distrito_show" id="distrito_show"
    ...
    @if (count($equipeprofissionais))
        ...
        <div class="container-fluid">
                    ...
                    <tbody>
                        @foreach ($equipeprofissionais as $equipeprofissional)
                            <tr>
                                ...
                                <td>
                                    @if (!empty($equipeprofissional->profissional->id))
                                        <span>
                                ...
                                <td>
                                    @isset($equipeprofissional->profissional->matricula)
                            ...
                            <td>
                                @isset($equipeprofissional->profissional->cpf)
            ...
            <td>
                @isset($equipeprofissional->profissional_id)
                    <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modallimparvaga"
                        ...
                        data-equipeprofissional-id="{{ $equipeprofissional->id }}" data-equipe-id="{{ $equipe->id }}">
                        <x-icon icon='x-circle' /> Remover
                    </button>
                @else
                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                        data-bs-target="#modalvincularprofissional" data-equipeprofissional-id="{{ $equipeprofissional->id }}"
                        data-cargo-id="{{ $equipeprofissional->cargo->id }}"
                        data-cargo-nome="{{ $equipeprofissional->cargo->nome }}" data-equipe-id="{{ $equipe->id }}">
            ...
            @else
                ...
                <div class="modal fade" id="modalvincularprofissional" tabindex="-1" aria-labelledby="JanelaVincularVaga"
                    aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="ModalLabelVincularVaga"><x-icon icon='plus-circle' /> Atribuir
                                    Profissional</h1>
                            ...
                            <div class="modal-body">
                                ...
                                <form method="POST" action="{{ route('equipegestao.preenchervaga') }}">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <input type="hidden" id="equipeprofissional_id" name="equipeprofissional_id"
                                            value="">
                                        <input type="hidden" id="profissional_id" name="profissional_id"
                                            value="{{ old('profissional_id') ?? '' }}">
                                        <input type="hidden" id="cargo_profissional_id" name="cargo_profissional_id"
                                            value="{{ old('cargo_profissional_id') ?? '' }}">


                                        <div class="col-12">
                                            <label for="profissional_nome" class="form-label">Profissional</label>
                                            <input type="text" class="form-control typeahead" name="profissional_nome"
                                                id="profissional_nome" value="{{ old('profissional_nome') ?? '' }}"
                                                autocomplete="off">

                                        </div>

                                        <div class="col-12">
                                            <button type="submit" class="btn btn-primary"><x-icon icon='plus-circle' /> Atribuir
                                                Profissional</button>
                                        </div>
                                    </div>
                                </form>
                ...
                {{-- Modal Limpar Vaga --}}
                <div class="modal fade" id="modallimparvaga" tabindex="-1" aria-labelledby="JanelaVincularVaga"
                    aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="ModalLabelLimparVaga"><x-icon icon='x-circle' /> Remover
                                    Profissional</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form method="POST" action="{{ route('equipegestao.limparvaga') }}">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="cargo_nome_limpar" id="cargo_nome_limpar"
                                            readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="profissional_nome_limpar">Profissional Vinculado</label>
                                        <input type="text" class="form-control" name="profissional_nome_limpar"
                                            id="profissional_nome_limpar" readonly>
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <input type="hidden" id="equipeprofissional_id_limpar" name="equipeprofissional_id_limpar"
                                        value="">
                                    <input type="hidden" id="equipe_id_limpar" name="equipe_id_limpar" value="">
                                    <button type="submit" class="btn btn-warning"><x-icon icon='x-circle' /> Remover
                                        Profissional</button>
                                </form>
                            </div>
                            <div class="modal-footer">
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $('#modallimparvaga').on('show.bs.modal', function(e) {
                        ...
                        $(e.currentTarget).find('input[name="equipeprofissional_id_limpar"]').val(
                    ...
                    $("#profissional_nome").typeahead({
                        ...
                        templates: {
                            ...
                            suggestion: function(data) {
                                return '<div><div class="text-bg-primary"> ' + data.text +
                                    ' - <strong>Matrícula:</strong> ' + data.matricula + '</div>' +
                                    '<div class="text-bg-light mx-1">Cargo: <i>' + data.cargo +
                                    '</i></div></div>';
                            }
                        }
