...
<div class="container-fluid">
  ...
  <x-btn-group label='MenuPrincipal' class="py-1">
    ...
    <x-dropdown-menu title='Reports' icon='printer'>
      ...
      <li>
        <a class="dropdown-item" href="{{ route('equipes.export.xls', ['descricao' => request()->input('descricao'), 'numero' => request()->input('numero'), 'cnes' => request()->input('cnes'), 'ine' => request()->input('ine'), 'minima' => request()->input('minima'), 'unidade' => request()->input('unidade'), 'distrito' => request()->input('distrito'), 'tipo' => request()->input('tipo')]) }}"><x-icon icon='file-earmark-spreadsheet-fill' /> {{ __('Export') . ' XLS' }}</a>
      </li>
      <li>
        <a class="dropdown-item" href="{{ route('equipes.export.csv', ['descricao' => request()->input('descricao'), 'numero' => request()->input('numero'), 'cnes' => request()->input('cnes'), 'ine' => request()->input('ine'), 'minima' => request()->input('minima'), 'unidade' => request()->input('unidade'), 'distrito' => request()->input('distrito'), 'tipo' => request()->input('tipo')]) }}"><x-icon icon='file-earmark-spreadsheet-fill'/> {{ __('Export') . ' CSV' }}</a>
      </li>
      <li>
        <a class="dropdown-item" href="{{ route('equipes.export.pdf', ['descricao' => request()->input('descricao'), 'numero' => request()->input('numero'), 'cnes' => request()->input('cnes'), 'ine' => request()->input('ine'), 'minima' => request()->input('minima'), 'unidade' => request()->input('unidade'), 'distrito' => request()->input('distrito'), 'tipo' => request()->input('tipo')]) }}"><x-icon icon='file-pdf-fill' /> {{ __('Export') . ' PDF' }}</a>
      </li>
            ...
            <tr>
                ...
                <td>
                  <x-btn-group label='Opções'>
                    ...
                    @can('equipe.edit')
                    <a href="{{ route('equipes.edit', $equipe->id) }}" class="btn btn-primary btn-sm" role="button"><x-icon icon='pencil-square'/></a>
                    @endcan

                    @can('equipe.show')
                    <a href="{{ route('equipes.show', $equipe->id) }}" class="btn btn-info btn-sm" role="button"><x-icon icon='eye'/></a>
                    @endcan
